...
// =========================================================
// NUEVA LÓGICA DE UART PARA QEMU (PL011)
// =========================================================

void uart_putc(char c) {
    // Espera si el buffer de transmisión está lleno (TXFF = Transmit FIFO Full)
    while (GET32(UART_FR) & UART_FR_TXFF);
    
    // Escribe el caracter en el Data Register
    PUT32(UART_DR, c);
}

char uart_getc(void) {
    // Espera si el buffer de recepción está vacío (RXFE = Receive FIFO Empty)
    while (GET32(UART_FR) & UART_FR_RXFE);
    
    // Lee el caracter del Data Register
